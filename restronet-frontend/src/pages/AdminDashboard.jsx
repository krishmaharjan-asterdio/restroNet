...
const StatSkeleton = () => (
  <div className="bg-white dark:bg-[#131e35] rounded-2xl border border-slate-200 dark:border-[#1e2d47] h-36 animate-pulse" />
);
    ...
    return (
      <div className="flex items-center justify-center h-full min-h-[400px] bg-slate-50 dark:bg-[#0f1629]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-slate-500 dark:text-[#8b98b0] text-sm">Loading overview…</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 max-w-[1600px] mx-auto">
      ...
      >
        ...
        {/* Admin profile card */}
        <div className="flex items-center gap-3 bg-white dark:bg-[#131e35] border border-slate-200 dark:border-[#1e2d47] px-4 py-3 rounded-2xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100">High Performers</h2>
            </div>
              ...
              trendingVenues.map((venue, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className="flex items-center gap-3.5 px-3.5 py-3 rounded-xl hover:bg-slate-100 dark:hover:bg-[#1e2d47] transition-colors duration-200 cursor-default"
                  >
                    {/* Logo */}
                    <div className="w-12 h-12 rounded-xl overflow-hidden border border-slate-200 dark:border-[#1e2d47] shrink-0 bg-slate-50 dark:bg-[#0f1629]">
                      {logoUrl ? (
                        <img
                          src={logoUrl}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-500 dark:text-[#8b98b0] font-medium mt-0.5">
                        {venue.totalReviews} reviews
                      </p>
                    </div>

                    {/* Rating badge */}
                    <div className="bg-emerald-50 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-400 rounded-full px-2.5 py-1 text-xs font-semibold flex items-center gap-1 shrink-0">
        ...
        >
          <div className="flex items-center gap-2.5 mb-6">
            ...
            <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100">Latest Feedback</h2>
          </div>
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100">Recently Added</h2>
            </div>
            <span className="text-[10px] font-bold text-slate-400 dark:text-[#4a5a78] uppercase tracking-widest">
              New Venues
            </span>
          </div>
            ...
            {recentVenues.map((venue, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex items-center gap-3 px-3.5 py-3 rounded-xl hover:bg-slate-100 dark:hover:bg-[#1e2d47] transition-colors duration-200 cursor-default"
                >
                  <div className="w-10 h-10 rounded-xl overflow-hidden border border-slate-200 dark:border-[#1e2d47] shrink-0 bg-slate-50 dark:bg-[#0f1629]">
                    {logoUrl ? (
                      <img src={logoUrl} className="w-full h-full object-cover" alt={venue.name} />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-slate-500 dark:text-[#8b98b0] font-bold text-sm">
                        {venue.name.charAt(0)}
                      </div>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-slate-800 dark:text-slate-100 truncate text-sm">{venue.name}</h4>
                    <p className="text-xs text-slate-500 dark:text-[#8b98b0] mt-0.5">
